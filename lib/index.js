...
// ✅ Custom session ID generator with "okazeus~" prefix
function okazeusId(num = 22) {
  let result = "";
  ...
  return `okazeus~${result}`;
}

async function downloadCreds(sessionId) {  
  try {
    if (!sessionId.startsWith('okazeus~')) {
      throw new Error('Invalid SESSION_ID: It must start with "okazeus~"');
    }
...
module.exports = { 
  downloadCreds, 
  removeFile, 
  okazeusId  // <-- Note: Exported with new name
};
